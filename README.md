...
发布后会匿名下载核对安装器、回执与实际更新源，并等待官网下载页显示本次版本、主下载与备用镜像链接。
官网的版本元数据有 300 秒缓存，验收允许 6 分钟刷新窗口；随后完整下载镜像并核对安装器 SHA-256。
这些检查全部通过后工作流才报告成功，结果记录在运行 summary 中。观察失败可沿既有恢复入口重试。
